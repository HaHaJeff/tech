...
**思路：环形相关，所以需要两个dp函数，一个是从0出发的，一个是从1出发的，因为选择最后一个是不能从0出发的**
...
**思路：同house robber一样，采用dp的思路，不过对于树而言，应该采取自顶向下的递归解决**
...
public:
    ...
    vector<int> helper(TreeNode* root) {
        ...
        // 选当前root，最大值为上一次不选当前root或者上前选了当前root+root->val
...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int K) {
        ...
        int ans = INT_MAX;
        dfs(graph, 0, ans, src, dst, K+1);
        return ans == INT_MAX ? -1 : ans;
        
    }
    
    void dfs(unordered_map<int, vector<pair<int, int>>>& graph, int cost, int& ans, int start, int end, int k) {
        if (start == end) {
        ...
        for (auto& p : graph[start]) {
            ...
            dfs(graph, cost + p.second, ans, p.first, end, k-1);
        }
    }
};
```

**思路：动态规划解决 dp[ k ] [ j ] = dp[ k-1 ] [ j ] + x**
``` cpp

class Solution {
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dst, int K) {
        int len = flights.size(); const int inf = 100 * 10000;
        vector<vector<int>> dp(K+2, vector<int>(len+1, inf));
        dp[0][src] = 0;
...
public:
    ...
    int uniquePathsIII(vector<vector<int>>& grid) {
        ...
        return dfs(si, sj, 1, total, grid);
    }
    
    int dfs(int si, int sj, int done, int total, vector<vector<int>>& grid)
    {
        if (si < 0 || si >= grid.size() || sj < 0 || sj >= grid[0].size() || grid[si][sj] == -1) return 0;
        if (grid[si][sj] == 2) return done == total ? 1 : 0;
        grid[si][sj] = -1;
        int ans = 0;
        for (auto& p : dir)
        {
            int i = si + p[0], j = sj + p[1];
            ans += dfs(i, j, done+1, total, grid);
